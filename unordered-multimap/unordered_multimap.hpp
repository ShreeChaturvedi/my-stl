...
public:
  ...
  using const_iterator = base_iterator<const value_type, typename ForwardList<value_type>::const_iterator>;

  unordered_multimap() : unordered_multimap(16) {}

  explicit unordered_multimap(size_type bucket_count) : buckets_{}, size_(0), max_load_factor_(1.0f) {
  ...
  std::pair<iterator, iterator> equal_range(const K& key) {
    ...
    if (it == buckets_[b].end()) return {end(), end()};

    auto last = it;
    while (last != buckets_[b].end() && KeyEqual{}(last->first, key)) ++last;
  ...
  std::pair<const_iterator, const_iterator> equal_range(const K& key) const {
    const size_type b = bucket_of(key);
    auto it = find_in_bucket(b, key);
    if (it == buckets_[b].end()) return {cend(), cend()};
...
private:
  ...
  void insert_into(Buckets& buckets, value_type value) {
  ...
  typename ForwardList<value_type>::iterator find_in_bucket(size_type b, const K& key) {
    auto& list = buckets_[b];
    for (auto it = list.begin(); it != list.end(); ++it) {
      if (KeyEqual{}(it->first, key)) return it;
    }
    return list.end();
  }
...
public:
  ...
  using map_ptr = std::conditional_t<std::is_const_v<T_value>, const unordered_multimap*, unordered_multimap*>;
  ...
  static base_iterator end(map_ptr map) { return base_iterator(map, map->bucket_count(), {}); }

  static base_iterator from_bucket(map_ptr map, std::size_t bucket, T_list_iterator it) {
    base_iterator out(map, bucket, it);
    if (bucket >= map->bucket_count()) return end(map);
...
private:
  ...
  base_iterator(map_ptr map, std::size_t bucket, T_list_iterator it) : map_(map), bucket_(bucket), it_(std::move(it)) {}
