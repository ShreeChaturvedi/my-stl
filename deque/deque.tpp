...
template <typename T>
T& Deque<T>::operator[](size_type i) noexcept {
  return data_[phys_index(i)];
}

template <typename T>
const T& Deque<T>::operator[](size_type i) const noexcept {
  return data_[phys_index(i)];
}

template <typename T>
T& Deque<T>::at(size_type i) {
  if (i >= size_) throw std::out_of_range("Deque::at out of range");
  return (*this)[i];
}

template <typename T>
const T& Deque<T>::at(size_type i) const {
  if (i >= size_) throw std::out_of_range("Deque::at out of range");
  return (*this)[i];
}
...
template <typename T>
T& Deque<T>::front() {
  if (empty()) throw std::out_of_range("Deque::front on empty");
  return (*this)[0];
}
...
template <typename T>
T& Deque<T>::back() {
  if (empty()) throw std::out_of_range("Deque::back on empty");
  return (*this)[size_ - 1];
}
...
template <typename T>
void Deque<T>::ensure_capacity_for_one_more() {
...
template <typename T>
void Deque<T>::grow(size_type new_capacity) {
  T* next = alloc_.allocate(new_capacity);

  if constexpr (std::is_nothrow_move_constructible_v<T>) {
    for (size_type i = 0; i < size_; ++i) std::construct_at(next + i, std::move((*this)[i]));
  } else {
    for (size_type i = 0; i < size_; ++i) std::construct_at(next + i, (*this)[i]);
  }
...
template <typename T>
void Deque<T>::destroy_all() noexcept {
  for (size_type i = 0; i < size_; ++i) std::destroy_at(data_ + phys_index(i));
}

template <typename T>
void Deque<T>::deallocate() noexcept {
  if (!data_) return;
